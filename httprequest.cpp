    ...
    : QObject(parent), manager(nullptr)
{
    ...
    //设置SSL证书
    //QList<QSslCertificate> certs = QSslCertificate::fromPath("/setting/pki-resource.cer");
    QList<QSslCertificate> certs = QSslCertificate::fromPath(QApplication::applicationDirPath()+"/pki-resource.cer");
...
void HttpRequestWorker::execute(HttpRequestInput *input) {
    ...
    if(input->request_body.size() > 0) {
        ...
        //QString new_line = "\r\n";
        //request_content.append(new_line);
        //request_content.append("application/xml; charset=UTF-8");
       // request_content.append(input->request_body);
    ...
    //request.setRawHeader("Authorization", "Basic " + QByteArray(QString("%1:%2").arg("admin%40internal").arg("hstecs123456").toLatin1()).toBase64());
